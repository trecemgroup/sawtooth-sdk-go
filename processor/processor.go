...
func (self *TransactionProcessor) start(context *zmq.Context) (bool, error) {
	...
	listener, err := messaging.NewConnection(context, zmq.DEALER, self.bind, true)
	if err != nil {
	...
	workersValidator, err := messaging.NewConnection(context, zmq.ROUTER, "inproc://validator", true)
	workersListener, err := messaging.NewConnection(context, zmq.ROUTER, "inproc://listener", true)
	if err != nil {
	...
	// Make work queues. Buffer so the router doesn't block
	queueValidator := make(chan *validator_pb2.Message, self.maxQueue)
	queueListener := make(chan *validator_pb2.Message, self.maxQueue)
	...
	for _, handler := range self.handlers {
		for _, version := range handler.FamilyVersions() {
			logger.Debugf(
				...
				version,
				handler.Namespaces())
			err := register(validator, shutdown_rx, handler, version, queueValidator, uint32(self.maxQueue))
			if err != nil {
	...
	for {
		...
		for _, ready := range polled {
			...
			case validator.Socket():
				receiveMessage(ids, validator, workersValidator, queueValidator)

			case listener.Socket():
				receiveMessage(ids, listener, workersListener, queueListener)

			case monitor:
				restart = receiveMonitor(monitor, self.shutdown_tx)

			case workersValidator.Socket():
				receiveWorkers(ids, validator, workersValidator)

			case workersListener.Socket():
...
func receiveWorkers(ids map[string]string, source, workers messaging.Connection) {
	...
	// Pass the message back to the source
	err = source.SendData("", data)
	if err != nil {
		logger.Errorf("Failed to send message (%v) to source: %v", corrId, err)
		return
	}
}
